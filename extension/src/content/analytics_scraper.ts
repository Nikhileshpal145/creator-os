  ...
  const scrapeLinkedInData = () => {
    try {
      ...
      const isAnalyticsDashboard =
        pageUrl.includes("/dashboard") || pageUrl.includes("/analytics");
      ...
      if (isProfilePage) {
        ...
        const connectionEl = document.querySelector(
          '[href*="connections"] span, .pv-top-card--connections span',
        );
        ...
        const nameEl = document.querySelector(
          "h1.text-heading-xlarge, h1.inline",
        );
      ...
      if (isAnalyticsDashboard) {
        const viewsEl = document.querySelector(
          '[aria-label*="Views"], .analytics-value-views',
        );
        const impressionsEl = document.querySelector(
          '[aria-label*="Impressions"], .analytics-value-impressions',
        );
...
chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
  ...
  if (req.action === "EXTRACT_PROFILE_DATA") {
    ...
    try {
      ...
      console.error("Creator OS: Extraction failed", errMsg);
...
function extractProfileData() {
  ...
  console.log("Creator OS: Extracted", {
    name,
    headline,
